...
void Mesh::draw(ShaderProgram* shaderProg, Camera* c, std::vector<std::vector<glm::vec3>> lightsData) {
	// temp solution	
	shaderProg->sendMatrixUniforms(true, this->transformationMatrices, glUniformMatrix4fv, 1);
	// view
	shaderProg->sendMatrixUniforms(false,
	...
	if (lightsData[0].size() > 0) {
		...
		for (int i = 0; i < lightsData[0].size(); i++) {
			...
			shaderProg->sendVecUniforms(false,
		...
		// send camera coords
		shaderProg->sendVecUniforms(false, std::unordered_map<std::string, glm::vec3>{ {"cameraCoords", c->cameraPosVector}}, glUniform3fv, 1);
	}
